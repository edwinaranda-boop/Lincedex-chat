...
var fondosDisponibles = ["bg-aqua", "bg-aurora", "bg-cosmic"];
var fondoActualIndice = 0;
var typingTimeout;
var miAvatarActual = "🦋"; // Avatar inicial por defecto
...
function reproducirSonido(tipo) {
    try {
        ...
        if (tipo === 'msg') { 
            ...
            gain1.gain.linearRampToValueAtTime(0.01, ctx.currentTime + 0.4);
...
// --- 🦋 SISTEMA DE AVATARES DE RESPALDO (ALEATORIO ANTIGUO) ---
...
// --- 🕹️ CONTROL DEL DOCK DE AVATARES ---
...
function renderizarMensaje(snapshot) {
    ...
    // RENDERIZAR AVATAR MANUAl O DE RESPALDO
    ...
    if (!divAvatar) {
        ...
        divAvatar.className = "avatar-glass";
        row.appendChild(divAvatar);
    }
...
// --- 🕹️ INTERRUPTOR DEL PICKER FLOTANTE ---
...
// --- 🟢 PRESENCIA Y MONITOR DE USUARIOS ONLINE ---
function actualizarPresencia() {
    var nombre = usernameInput.value.trim() || "Anónimo";
    if (nombre === CLAVE_ADMIN) nombre = "🛡️ ADMINISTRADOR";
    var status = estadosDisponibles[estadoActualIndice];
    
    var refUser = database.ref('presence/' + miSessionId);
    refUser.set({ usuario: nombre, estado: status, avatar: miAvatarActual });
    refUser.onDisconnect().remove();
}

database.ref('.info/connected').on('value', function(snapshot) {
    if (snapshot.val() === true) { actualizarPresencia(); }
});
...
// --- 🟢 BOTÓN GEMA DE CAMBIO DE ESTADO ---
function ciclarEstado() {
    ...
    var nuevoEstado = estadosDisponibles[estadoActualIndice];
    myStatusGemma.classList.add(nuevoEstado);
    actualizarPresencia();
}

// --- ✏️ DETECTOR DE "ESCRIBIENDO..." ---
...
database.ref('typing').on('value', function(snapshot) {
    ...
    snapshot.forEach(function(child) {
        if (child.key !== miSessionId) {
            listaEscribiendo.push(child.val().usuario);
        }
    });
...
usernameInput.addEventListener('input', function() {
    ...
    database.ref('mensajes').once('value', function(snapshot) {
        snapshot.forEach(function(childSnapshot) { renderizarMensaje(childSnapshot); });
    });
});

// --- ✉️ ENVÍO DE CONTENIDOS ---
...
function eliminarMensaje(idMensaje) {
    if (usernameInput.value.trim() === CLAVE_ADMIN) {
        database.ref('mensajes/' + idMensaje).remove();
    }
}

// --- 📡 ESCUCHADORES DE FIREBASE ---
database.ref('mensajes').on('child_added', function(snapshot) {
    renderizarMensaje(snapshot);
});

database.ref('mensajes').on('child_changed', function(snapshot) {
    renderizarMensaje(snapshot); 
});
